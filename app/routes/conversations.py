    ...
    with Session(engine) as session:
        if data.conversation_type == ChannelType.DIRECT:
            if not data.participant_id:
                raise HTTPException(status_code=400, detail="participant_id required for DM conversations")
            ...
            # Creating a channel
            if not data.name:
                raise HTTPException(status_code=400, detail="name required for channel conversations")
            if not data.workspace_id:
                raise HTTPException(status_code=400, detail="workspace_id required for channel conversations")
...
async def get_recent_conversations(
    ...
    with Session(engine) as session:
        ...
        return [ConversationInfo.model_validate(conv.model_dump()) for conv in conversations] 